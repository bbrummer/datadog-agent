...
func TestDbusConnectionErr(t *testing.T) {
	...
	mockSender.AssertCalled(t, "ServiceCheck", canConnectServiceCheckName, metrics.ServiceCheckCritical, "", []string(nil), expectedErrorMsg)
...
func TestServiceCheckSystemState(t *testing.T) {
	data := []struct {
		...
		{"degraded", metrics.ServiceCheckWarning, "Systemd status is \"degraded\""},
		{"maintenance", metrics.ServiceCheckUnknown, "Systemd status is \"maintenance\""},
		{"stopping", metrics.ServiceCheckUnknown, "Systemd status is \"stopping\""},
	...
	for _, d := range data {
		t.Run(fmt.Sprintf("state %s should be mapped to %s", d.systemStatus, d.expectedServiceCheckStatus.String()), func(t *testing.T) {
			...
			mockSender.AssertCalled(t, "ServiceCheck", canConnectServiceCheckName, d.expectedServiceCheckStatus, "", []string(nil), d.expectedMessage)
		})
	}
}
...
func TestSendPropertyAsGaugeSkipAndWarnOnMissingProperty(t *testing.T) {
	...
	serviceUnitConfigCPU := serviceUnitConfig{metricName: "systemd.unit.cpu_usage_n_sec", propertyName: "CPUUsageNSec", accountingProperty: "CPUAccounting", required: true}
	serviceUnitConfigNRestart := serviceUnitConfig{metricName: "systemd.unit.n_restarts", propertyName: "NRestarts", accountingProperty: "", required: true}
	...
	mockSender.AssertCalled(t, "Gauge", "systemd.unit.cpu_usage_n_sec", float64(110), "", []string(nil))
	mockSender.AssertNotCalled(t, "Gauge", "systemd.unit.n_restarts", mock.Anything, mock.Anything, mock.Anything)
}
